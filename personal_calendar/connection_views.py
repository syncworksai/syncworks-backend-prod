...
class CalendarConnectionListView(APIView):
    ...
    def get(self, request):
        return Response({
            ...
            "providers": {
                "google": bool(__import__("os").getenv("GOOGLE_CALENDAR_CLIENT_ID")),
                "microsoft": bool(__import__("os").getenv("MICROSOFT_CALENDAR_CLIENT_ID")),
                "apple": False,
            },
        })
    ...
    def post(self, request):
        ...
        try:
            if provider == "GOOGLE":
                ...
                url = microsoft.authorization_url(state)
            else:
                return Response({"detail": "Apple/iOS connection is not available in this build."}, status=400)
...
class CalendarOAuthCallbackView(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            target = _frontend_redirect(state.get("return_to"), "connected", provider)
            return HttpResponseRedirect(target)
        except Exception:
            target = _frontend_redirect("/customer/settings", "error", self.provider)
            return HttpResponseRedirect(target)
...
class CalendarConnectionSyncView(APIView):
    ...
    def post(self, request, connection_id):
        ...
        if not connection:
            return Response({"detail": "Calendar connection not found."}, status=404)
        result = sync_connection(request.user, connection)
