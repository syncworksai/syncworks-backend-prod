...
def _jarvis_setup_url(frontend: str, checkout: str | None = None) -> str:
    base = f"{frontend}/upgrade?product=jarvis"
    if checkout:
        return f"{base}&checkout={checkout}"
    return base
...
class UserJarvisCheckInView(APIView):
    ...
    def post(self, request):
        ...
        AuditLog.objects.create(
            actor=request.user,
            action="jarvis.day.checked_in",
            metadata={"local_date": today},
        )
...
class UserJarvisCheckOutView(APIView):
    ...
    def post(self, request):
        ...
        AuditLog.objects.create(
            actor=request.user,
            action="jarvis.day.checked_out",
            metadata={"local_date": today, "reason": reason},
        )
        return Response(
            {
                "local_date": today,
...
class UserJarvisCheckoutView(APIView):
    ...
    def post(self, request):
        ...
        if not secret or not price_id:
            return Response(
                {"detail": "Jarvis billing is not configured yet."},
                status=503,
            )
        ...
        frontend = (
            os.getenv("FRONTEND_URL") or "https://syncworksapp.com"
        ).rstrip("/")

        args = {
            "mode": "subscription",
            "line_items": [{"price": price_id, "quantity": 1}],
            "success_url": _jarvis_setup_url(frontend, "success"),
            "cancel_url": _jarvis_setup_url(frontend, "cancelled"),
...
class UserJarvisPortalView(APIView):
    ...
    def post(self, request):
        ...
        if not secret or not billing.stripe_customer_id:
            return Response(
                {"detail": "No billing account is available yet."},
                status=400,
            )
        ...
        portal = stripe.billing_portal.Session.create(
            ...
            return_url=_jarvis_setup_url(frontend),
        )
...
class UserJarvisWebhookView(APIView):
    ...
    def post(self, request):
        ...
        if event["type"] == "checkout.session.completed" and user_id:
            ...
            save_profile(
                user,
                {"plan": plan, "onboarding_complete": True},
            )
            billing, _ = UserBillingProfile.objects.get_or_create(user=user)
            billing.stripe_customer_id = (
                obj.get("customer") or billing.stripe_customer_id
            )
            billing.stripe_subscription_id = (
                obj.get("subscription") or billing.stripe_subscription_id
            )
            billing.subscription_status = "active"
            billing.save()
            AuditLog.objects.create(
                actor=user,
                action="jarvis.subscription.activated",
                metadata={"plan": plan},
            )
