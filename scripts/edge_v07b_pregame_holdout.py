...
from edge_v07_pregame_holdout import (
    mlb_games, markets_for_day, market_team, play_states, candles, latest_pregame_ask,
    candle_ask, fit, apply, brier, strategy,
)


def run(days=20):
    end=date.today()-timedelta(days=1); start=end-timedelta(days=days-1)
    obs=[]; cov={"days":days,"games":0,"matched_games":0,"markets":0,"candles":0,"pregame_baselines":0,"errors":[]}
    for i in range(days):
        ...
        for game_pk,away,home,gstart in games:
            ...
            hist={m["ticker"]:candles(m,gstart-5400,times[-1]+600) for m in matched}
            cov["candles"]+=sum(len(v) for v in hist.values())
            side_market={market_team(m):m for m in matched}; am=side_market.get(away); hm=side_market.get(home)
            if not am or not hm: continue
            # Kalshi candles often begin after scheduled first-pitch time. The latest quote
            # at or before the first completed MLB play is still pre-state information.
            ...
            time.sleep(.02)
    dates=sorted({r["date"] for r in obs}); cut=max(1,int(len(dates)*.60))
    train_dates=set(dates[:cut]); hold_dates=set(dates[cut:])
    ...
    result={"period":{"start":str(start),"end":str(end)},"coverage":cov,"model":{"version":"EDGE-MLB-v0.7b-pregame-context","pregame_baseline":"normalized last Kalshi asks at/before first completed MLB play","fit":params,"split":"chronological 60% train / 40% untouched holdout"},"results":{"train":pack(train),"holdout":pack(hold)},"limitations":["Research only; no live orders.","Friction modeled as +1.5 cents to entry price.","Pregame baseline uses last quote before first completed play when scheduled-time candles are absent.","Pitcher, bullpen, lineup and injuries are not yet included."]}
