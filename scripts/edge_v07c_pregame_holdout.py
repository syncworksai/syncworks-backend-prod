...
from edge_v07_pregame_holdout import KALSHI, get_json, cents, dt, mlb_games, markets_for_day, market_team, play_states, candles, latest_pregame_ask, candle_ask, fit, apply, brier, strategy
...
def last_trade_before(ticker, start_ts, cutoff_ts):
    ...
    for trade in trades:
        ...
        if when and when.timestamp()<=cutoff_ts and price is not None and 1<=price<=99: valid.append((when.timestamp(),price))
    ...
    for i in range(days):
        ...
        for game_pk,away,home,gstart in games:
            ...
            if ap is None or hp is None:
                ap=last_trade_before(am["ticker"],gstart-86400,first_state_ts); hp=last_trade_before(hm["ticker"],gstart-86400,first_state_ts); source="trade"
