        ...
        rows.append(
            {
                ...
                "trouble_rate": (
                    st["hits"] + st["walks_hbp"]
                ) / bf,
                "strikeout_rate": st["strikeouts"] / bf,
                "relievers_used": max(
                    0,
                    len(pitchers_seen[defense]) - 1,
                ),
...
def enrich_advanced(obs):
    ...
    for game_pk, game_rows in by_game.items():
        ...
        for row in game_rows:
            ...
            defensive_team = (
                row["home"]
                if state["defense"] == "home"
                else row["away"]
            )

            sign = (
                1.0
                if row["side"] == defensive_team
                else -1.0
            )
...
def fair_f3(row, cfg):
    ...
    sign = float(
        row.get("pitching_sign") or 0.0
    )

    if sign:
        trouble = float(
            row.get("trouble_rate") or 0.0
        )

        k_rate = float(
            row.get("strikeout_rate") or 0.0
        )

        relievers = float(
            row.get("relievers_used") or 0.0
        )

        runners = float(
            row.get("runners_on_base") or 0.0
        )
...
def replay(rows, model, cfg, threshold):
    ...
    for row in sorted(
        ...
        if (
            edge < threshold
            or ask + f.ENTRY_FRICTION_CENTS >= 100
        ):
            continue
    ...
    return {
        ...
        "roi_pct": (
            round(
                100 * sum(pnl) / len(trades),
                2,
            )
            if trades
            else None
        ),
        "max_drawdown_units": round(
            max_dd,
            3,
        ),
        "avg_model_edge_pct": (
...
def select_threshold(validation):
    eligible = [
        ...
        for t, stats in validation.items()
        if stats["trades"]
...
def model_result(
    ...
    val_thresholds = validation_pack(
        val,
        model,
        cfg,
    )
...
def run(days=60):
    f.source.markets_for_day = (
        f.archive.combined_markets_for_day
    )
    ...
    (
        ...
        obs,
        coverage,
    ) = f.source.corrected_build_observations(
        days
    )

    pitcher_coverage = (
        f.enrich_pitchers(obs)
    )
    ...
    dev = [
        r
        for r in obs
        if r["date"] in dev_dates
    ]

    val = [
        r
        for r in obs
        if r["date"] in val_dates
    ]

    hold = [
        r
        for r in obs
        if r["date"] in hold_dates
    ]
    ...
    result = {
        ...
        "selection_rule": (
            "Model coefficients fit on development only. "
            "Entry threshold chosen by validation ROI with "
            ">=40 trades when available. Holdout is evaluated "
            "only at the validation-selected threshold."
        ),
        "models": {
            "F1_GAME_STATE": model_result(
                "F1 game-state",
                "F1",
                f1,
                dev,
            ...
            "F2_PITCHER_AWARE": model_result(
                "F2 pitch-count/fatigue",
                "F2",
                f2,
                dev,
                val,
            ...
            "F3_BASEBALL_CONTEXT": model_result(
                (
                    "F3 pitcher performance + "
                    "base pressure + bullpen usage"
                ),
                "F3",
                f3,
                dev,
        ...
        "guardrails": [
            ...
            (
                "One simulated entry per game "
                "per model/threshold."
            ),
            "Entry friction +1.5 cents.",
            (
                "No holdout threshold "
                "optimization."
            ),
            (
                "A/B/E1/E2 frozen experiment "
                "is untouched."
            ),
        ],
    }
    ...
    Path(
        "edge_v17_strategy_f3_results.json"
    ).write_text(
        json.dumps(
            result,
            indent=2,
