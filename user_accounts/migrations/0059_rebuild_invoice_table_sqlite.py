# backend/user_accounts/migrations/0059_rebuild_invoice_table_sqlite.py
...
def rebuild_invoice_table(apps, schema_editor):
    ...
    # Create the NEW (business-based) invoice table that matches your current Invoice model (billing.py)
    ...
    if has_old:
        ...
        # CASE B: old is Ticket-based invoice schema (has ticket_id, total/status/title/notes)
            ...
            status_expr = "'OPEN'"
            if has_status:
                # IMPORTANT: qualify i.status to avoid ambiguity with ticket.status
                status_expr = """
                CASE
