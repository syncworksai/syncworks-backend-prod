...
from platform_growth.services.runtime import prepare_due_scheduled_posts, run_due_recipes


class Command(BaseCommand):
    help = "Run due SyncWorks Growth bot recipes and prepare approved scheduled posts."
    ...
    def handle(self, *args, **options):
        ...
        for recipe_id, result in recipe_results:
            self.stdout.write(
                f"recipe={recipe_id} status={result.status} message={result.message}"
            )
