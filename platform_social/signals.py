...
def _calendar_defaults(event, existing=None):
    ...
    return {
        ...
        "status": (
            ...
            else PersonalCalendarEvent.Status.ACTIVE
        ),
        "metadata": metadata,
    }
...
def sync_event_to_user_calendar(event, user_id):
    """Idempotently mirror one Social event into one user's Personal Calendar."""
    lookup = {
        "owner_id": user_id,
    ...
    if changed_fields:
        ...
        action = (
            ...
            if existing.status == PersonalCalendarEvent.Status.CANCELLED
            else PersonalCalendarEventAudit.Action.UPDATED
        )
        PersonalCalendarEventAudit.objects.create(
            event=existing,
            actor=event.created_by,
...
def calendar_recipient_user_ids(event):
    ...
    accepted_group_ids = GroupEventInvitation.objects.filter(
        event=event,
...
def sync_event_calendars(event):
    """Propagate Social edits/cancellation to every linked and currently eligible user."""
