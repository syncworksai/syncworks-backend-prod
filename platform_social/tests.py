...
from .models import Collection, EventMemberResponse, GroupEventInvitation, GroupMembership, SocialEvent, SocialGroup
...
class SocialApiPermissionTests(APITestCase):
    ...
    def social_calendar_rows(self, event=None):
        ...
        return PersonalCalendarEvent.objects.filter(source=PersonalCalendarEvent.Source.SYNC, external_calendar_id="syncworks-social", external_event_id=f"social:{event.id}")

    def test_plain_member_cannot_patch_group_event_invitation(self):
        self.authenticate(self.member)
        response = self.client.patch(reverse("social-event-invitations-detail", args=[self.invitation.pk]), {"status": "ACCEPTED"}, format="json")
    ...
    def test_plain_member_cannot_edit_collection(self):
        self.authenticate(self.member)
        response = self.client.patch(reverse("social-collections-detail", args=[self.collection.pk]), {"total_amount_cents": 1}, format="json")
    ...
    def test_member_cannot_rsvp_until_group_accepts_event(self):
        self.authenticate(self.member)
        response = self.client.post(reverse("social-event-responses-list"), {"event": self.event.id, "group": self.team.id, "response": "YES"}, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.authenticate(self.manager)
        response = self.client.post(reverse("social-event-invitations-accept", args=[self.invitation.pk]), {}, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_new_active_member_gets_pending_rsvp_and_calendar_for_already_accepted_event(self):
        ...
        membership = GroupMembership.objects.create(group=self.team, user=late_member, role=GroupMembership.Role.MEMBER, status=GroupMembership.Status.INVITED, invited_by=self.manager)
    ...
    def test_member_cannot_create_child_group_under_group_they_do_not_manage(self):
        self.authenticate(self.member)
        response = self.client.post(reverse("social-groups-list"), {"name": "Unauthorized Child", "kind": "TEAM", "visibility": "PRIVATE", "parent": self.organization.id}, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_manager_can_update_collection_amount(self):
        self.authenticate(self.manager)
        response = self.client.patch(reverse("social-collections-detail", args=[self.collection.pk]), {"total_amount_cents": 45000}, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.collection.refresh_from_db()
